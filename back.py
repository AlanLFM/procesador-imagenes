...
def ecualizacion_log_hiperbolica(img):
    """Ecualización log-hiperbólica del histograma"""
    if len(img.shape) == 3:
...
def aplicar_roberts(img):
    ...
    roberts = cv2.sqrt(cv2.addWeighted(cv2.pow(roberts_x, 2.0), 1.0,
...
def aplicar_multiumbral(img, classes=3):
    """Aplicar multiumbralización usando Otsu"""
    if len(img.shape) == 3:
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
def extraer_canales_rgb(img):
    """Extraer canales RGB de la imagen"""
    if len(img.shape) == 3:
        b, g, r = cv2.split(img)
...
def calcular_histograma(img):
    ...
    if len(img.shape) == 3:
        # Para imágenes a color, calcular histogramas RGB
        b, g, r = cv2.split(img)
        hist_b = cv2.calcHist([b], [0], None, [256], [0, 256])
        hist_g = cv2.calcHist([g], [0], None, [256], [0, 256])
        hist_r = cv2.calcHist([r], [0], None, [256], [0, 256])
        
        # También calcular histograma de luminancia
...
def operaciones_aritmeticas(img, escalar):
    """Realizar operaciones aritméticas con escalar"""
    if len(img.shape) == 3:
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
def operaciones_logicas(img1, img2):
    """Realizar operaciones lógicas entre dos imágenes"""
    if len(img1.shape) == 3:
...
@app.route('/')
def index():
    return "Servidor de Procesamiento de Imágenes funcionando!"
...
def process_image():
    ...
    try:
        ...
        if operation == 'histograma':
            ...
            # Se necesita una segunda imagen
